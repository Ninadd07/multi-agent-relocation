...
def smooth_polyline(points, iterations=3):
    """Chaikin corner-cutting: smooth a polyline without moving endpoints."""
...
def save_map(obstacles, filename=None):
    ensure_dir("maps")

    if filename is None:
        index = 1
        while True:
            candidate = f"mars_map_{index}.json"
            filepath = os.path.join("maps", candidate)
...
def save_gif(frames, filename="mars_sim.gif"):
    ...
    try:
        ...
        print(f"[ERROR] Error saving GIF: {e}")
    ...
    for x in range(20, MAP_WIDTH, step):
        for y in range(20, MAP_HEIGHT, step):
            ...
            for ag in agents:
                ...
                dist = diff.length()
                if 1 < dist < 80:
                    force += diff.normalize() * (K_AGENT / (dist ** 2))
            for obs in obstacles:
                ...
                dist = diff.length()
                if 1 < dist < 60:
                    force += diff.normalize() * (K_WALL / (dist ** 2))
...
def apply_loaded_map(custom_obstacles, state, sim_left, sim_right):
...
def main():
    ...
    buttons += [
        ...
        Button(bx + 90, file_y, 80, 28, "Load Sel", "LOAD_SELECTED_MAP", color=BLUE),
        Button(bx, file_y + 35, 80, 28, "Prev Map", "PREV_MAP", color=GRAY_BG),
        Button(bx + 90, file_y + 35, 80, 28, "Next Map", "NEXT_MAP", color=GRAY_BG),
        Button(bx, file_y + 70, bw, 28, "Clear Walls", "CLEAR_WALLS", color=ORANGE),
    ]
    ...
    custom_obstacles = []
    saved_map_files = list_saved_maps()
    ...
    while running:
        ...
        mouse_pos = scale_mouse_pos(raw_mouse_pos, base_width, base_height, window_width, window_height)
        ...
        for event in pygame.event.get():
            ...
            if hasattr(event, "pos"):
                event_mouse_pos = scale_mouse_pos(event.pos, base_width, base_height, window_width, window_height)
            event_map_mouse_pos = (event_mouse_pos[0] - PANEL_WIDTH, event_mouse_pos[1])
            ...
            for sim, dropdown in [(sim_left, algo_dropdown_left), (sim_right, algo_dropdown_right)]:
                ...
                if dropdown.handle_event(event, event_mouse_pos):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and not dropdown_consumed:
                if event_mouse_pos[0] < PANEL_WIDTH:
                    for btn in buttons:
                        action = btn.check_click(event_mouse_pos)
                        if action:
                            if action == "RESTART":
                                state = "DRAWING"
                                is_recording = False
                                ...
                                is_recording = btn.active
                            elif action == "SAVE_GIF":
                                save_gif(recorded_frames)
                            elif action == "SAVE_MAP":
                                save_map(custom_obstacles)
                                saved_map_files = list_saved_maps()
                                if saved_map_files:
                                    selected_map_index = len(saved_map_files) - 1

                            elif action == "PREV_MAP":
                                saved_map_files = list_saved_maps()
                                if saved_map_files:
                                    selected_map_index = (selected_map_index - 1) % len(saved_map_files)
                            ...
                            elif action == "LOAD_SELECTED_MAP":
                                saved_map_files = list_saved_maps()
                                if not saved_map_files:
                                    print("[WARNING] No saved maps found in maps/")
                                else:
                                    filename = saved_map_files[selected_map_index]
                                    print(f"[INFO] Loading {filename}")
                                    loaded = load_map(filename)
                                    if loaded:
                                        custom_obstacles = loaded
                                        apply_loaded_map(custom_obstacles, state, sim_left, sim_right)
                ...
                elif event_map_mouse_pos[0] >= 0:
                    if active_tool == "TOOL_RECT":
                        current_drawing_rect = [event_map_mouse_pos[0], event_map_mouse_pos[1], 0, 0]
                    elif active_tool == "TOOL_CIRC":
                        current_drawing_circ = [event_map_mouse_pos[0], event_map_mouse_pos[1], 0]
                    elif active_tool == "TOOL_DRAW":
                        current_drawing_freehand = [event_map_mouse_pos]

            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
                if event_map_mouse_pos[0] >= 0:
                    to_remove = None
                    for obs in reversed(custom_obstacles):
                        if obs.collidepoint(event_map_mouse_pos):
                            to_remove = obs
                            break
                    if to_remove:
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                if current_drawing_rect:
                    current_drawing_rect[2] = event_map_mouse_pos[0] - current_drawing_rect[0]
                    current_drawing_rect[3] = event_map_mouse_pos[1] - current_drawing_rect[1]
                if current_drawing_circ:
                    r = math.hypot(event_map_mouse_pos[0] - current_drawing_circ[0], event_map_mouse_pos[1] - current_drawing_circ[1])
                ...
                if current_drawing_freehand is not None and len(current_drawing_freehand) > 0 and pygame.mouse.get_pressed()[0]:
                    ...
                    if math.hypot(last_point[0] - event_map_mouse_pos[0], last_point[1] - event_map_mouse_pos[1]) > 8:
                        current_drawing_freehand.append(event_map_mouse_pos)
                    ...
                    if sim_left.agents or sim_right.agents:
                        for sim in (sim_left, sim_right):
                            for agent in sim.agents:
                                a_rect = pygame.Rect(
                                    agent.pos.x - AGENT_RADIUS,
                                    agent.pos.y - AGENT_RADIUS,
                                    AGENT_DIAMETER,
                                    AGENT_DIAMETER,
                                )
        ...
        if state == "RUNNING":
            ...
            for sim in (sim_left, sim_right):
                if sim.all_parked or not sim.central_manager or not sim.agents:
                    continue
                ...
                for sim in (sim_left, sim_right):
                    ...
                    if not sim.use_electric:
                        if isinstance(sim.planner, QueuePlanner):
                            ...
                            for agent in sim.agents:
                                agent.local_safety_check(sim.agents)
            ...
            all_entered_both = (
                (all(a.spot_reserved for a in sim_left.agents) if sim_left.agents else False)
                and
        ...
        # --- DRAWING TO BASE SURFACE ---
        base_surface.fill(OFF_WHITE)
        pygame.draw.rect(base_surface, GRAY_BG, (0, 0, PANEL_WIDTH, SCREEN_HEIGHT))
        pygame.draw.line(base_surface, (60, 60, 60), (PANEL_WIDTH, 0), (PANEL_WIDTH, SCREEN_HEIGHT), 2)

        status_txt = state
        if not timer_running and state == "RUNNING":
            status_txt = "FINISHED"
        base_surface.blit(font_bold.render(f"STATUS: {status_txt}", True, TEXT_GRAY), (bx, 30))

        info_y = 65
        if state == "DRAWING":
            base_surface.blit(font_header.render("INSTRUCTIONS", True, TEXT_WHITE), (bx, info_y))
            base_surface.blit(font_ui.render("L-Click: Draw Wall", True, TEXT_GRAY), (bx, info_y + 20))
            base_surface.blit(font_ui.render("R-Click: Erase Wall", True, TEXT_GRAY), (bx, info_y + 35))
            base_surface.blit(font_bold.render("ENTER: Start Sim", True, GREEN), (bx, info_y + 60))
        else:
            base_surface.blit(font_header.render("LIVE EDITING", True, TEXT_WHITE), (bx, info_y))
            base_surface.blit(font_ui.render("Draw/Erase walls to", True, TEXT_GRAY), (bx, info_y + 20))
            base_surface.blit(font_ui.render("force re-routing!", True, TEXT_GRAY), (bx, info_y + 35))

        for btn in buttons:
            btn.draw(base_surface, font_ui)

        current_map_name = "No maps"
        if saved_map_files:
            current_map_name = saved_map_files[selected_map_index]
        display_map_name = current_map_name.replace(".json", "")
        if len(display_map_name) > 18:
            display_map_name = display_map_name[:15] + "..."
        base_surface.blit(font_ui.render(f"Selected: {display_map_name}", True, TEXT_GRAY), (bx, file_y + 102))

        base_surface.blit(font_label.render("LEFT MAP", True, TEXT_WHITE), (bx, algo_left_y - 15))
        base_surface.blit(font_label.render("RIGHT MAP", True, TEXT_WHITE), (bx, algo_right_y - 15))

        for sim, dropdown in [(sim_left, algo_dropdown_left), (sim_right, algo_dropdown_right)]:
            map_clip = pygame.Rect(sim.map_offset_x, 0, MAP_WIDTH, MAP_HEIGHT)
            base_surface.set_clip(map_clip)

            if sim == sim_right:
                pygame.draw.line(base_surface, (80, 80, 80), (sim.map_offset_x, 0), (sim.map_offset_x, SCREEN_HEIGHT), 2)

            s_rect = start_rect.move(sim.map_offset_x, 0)
            e_rect = end_rect.move(sim.map_offset_x, 0)
            pygame.draw.rect(base_surface, GREEN, s_rect, 2)
            pygame.draw.rect(base_surface, GREEN, e_rect, 2)

            lbl_s = font_label.render("START", True, GREEN)
            lbl_e = font_label.render("END", True, GREEN)
            base_surface.blit(lbl_s, (s_rect.x, s_rect.y - 18))
            base_surface.blit(lbl_e, (e_rect.x, e_rect.y - 18))
            ...
            pygame.draw.rect(base_surface, (30, 30, 30), bg_rect, border_radius=5)
            pygame.draw.rect(base_surface, (80, 80, 80), bg_rect, 1, border_radius=5)
            base_surface.blit(timer_surf, (tx, ty))

            fake_exit_manager = SmartExit(end_rect) if sim.exit_manager is None else sim.exit_manager
            draw_exit_grid(base_surface, fake_exit_manager, sim.map_offset_x)

            for obs in custom_obstacles:
                obs.draw(base_surface, BLUE, sim.map_offset_x, 0)
            ...
            if btn_show_grid.active:
                ...
                if isinstance(viz_planner, Discretisation):
                    ...
                    base_surface.blit(grid_surf, (sim.map_offset_x, 0))

            if current_drawing_rect:
                preview = [
                    current_drawing_rect[0] + sim.map_offset_x,
                    current_drawing_rect[1],
                    current_drawing_rect[2],
                    current_drawing_rect[3],
                ]
                temp_rect = pygame.Rect(preview)
                temp_rect.normalize()
                color = RED if input_blocked else (100, 100, 255)
                pygame.draw.rect(base_surface, color, temp_rect, 2)
            if current_drawing_circ:
                cx = current_drawing_circ[0] + sim.map_offset_x
                cy = current_drawing_circ[1]
                r = current_drawing_circ[2]
                color = RED if input_blocked else (100, 100, 255)
                pygame.draw.circle(base_surface, color, (int(cx), int(cy)), int(r), 2)
            if current_drawing_freehand is not None and len(current_drawing_freehand) > 1:
                ...
                color = RED if input_blocked else (100, 100, 255)
                pygame.draw.lines(base_surface, color, False, pts, 4)

            if state in ["RUNNING", "PAUSED"]:
                if sim.use_electric:
                    draw_electric_field(base_surface, sim.agents, custom_obstacles, end_rect, sim.map_offset_x)
                ...
                for agent in sim.agents:
                    ...
                    if len(agent.path) > 1 and agent.active:
                        ...
                        if len(display_points) > 1:
                            pygame.draw.lines(base_surface, YELLOW, False, display_points, 1)
                    pygame.draw.circle(base_surface, agent.get_color(), draw_pos, AGENT_RADIUS)
                    pygame.draw.circle(base_surface, BLACK, draw_pos, AGENT_RADIUS, 1)
                ...
                if sim.path_error:
                    ...
                    pygame.draw.rect(base_surface, (50, 50, 50), box_rect)
                    pygame.draw.rect(base_surface, RED, box_rect, 3)
                    txt1 = error_font.render("NO PATH FOUND!", True, RED)
                    base_surface.blit(txt1, (box_rect.centerx - txt1.get_width() // 2, box_rect.centery - 15))

        base_surface.set_clip(None)

        algo_dropdown_right.draw(base_surface, font_ui)
        algo_dropdown_left.draw(base_surface, font_ui)

        if is_recording and state == "RUNNING":
            pygame.draw.circle(base_surface, RED, (SCREEN_WIDTH - 30, 30), 8)
            if frame_counter % 2 == 0:
                recorded_frames.append(base_surface.copy())
            frame_counter += 1
