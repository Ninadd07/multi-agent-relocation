...
class Agent:
    ...
    def recalc_path(self):
        ...
        # The target for the A* path should be the smart parking corner, not the geometric center.
        ...
        if new_path:
            self.path = new_path
            # If the path has more than one point (start + at least one waypoint),
            # immediately target the first waypoint (index 1). This prevents the agent
            # from trying to move back to its own starting position (index 0) if it
            # gets pushed before its first move. This fixes the "double take" issue.
            if len(new_path) > 1:
    ...
    def resolve_collision(self, agents, obstacles):
        ...
        # We need to import the obstacle classes to check their type. This is a bit
        # of a circular dependency, but necessary to apply shape-specific physics.
        from algorithms.obstacles import CircleObstacle, FreehandObstacle

        for obs in obstacles:
            # --- START of FIX ---
            # The original code used a single AABB-based collision check, which failed for
            # non-rectangular shapes, creating "invisible walls". This new logic checks
            # the obstacle type and applies the correct collision math.
            if isinstance(obs, CircleObstacle):
                # Circle vs Agent(Circle) collision
                diff = self.pos - pygame.Vector2(obs.cx, obs.cy)
                dist = diff.length()
                min_dist = AGENT_RADIUS + obs.r
                if dist < min_dist:
                    if dist == 0: self.pos.x += 1 # Avoid division by zero
                    else:
                        overlap = min_dist - dist
                        self.pos += diff.normalize() * overlap

            elif isinstance(obs, FreehandObstacle):
                # Find the closest point on the entire freehand shape to the agent's center
            ...
            else: # Default to RectObstacle logic (or any other AABB-based shape)
        ...
        for other in agents:
            ...
            # Chain-reaction parking: if I'm in the zone and touch a parked agent, I also park.
            if self.spot_reserved and not other.active:
                if self.pos.distance_to(other.pos) < AGENT_DIAMETER:
                    ...
                    return # I am now parked, stop processing for this agent.
    ...
    def update(self, end_rect):
        ...
        if self.spot_reserved:
            # Physics-based continuous parking. Agents are gently pulled towards the
            # target corner and settle into a crowd via collision resolution.
            target = self.exit_manager.center_pixel
            diff = target - self.pos
            dist = diff.length()

            # If an agent is close to the center and has stopped, it's parked.
            # This helps the first few agents park quickly.
            if dist < AGENT_RADIUS and self.velocity.length() < 0.1:
                self.velocity.update(0, 0)
                self.active = False
                self.exit_manager.park_agent(self)
                return

            # Apply a gentle attraction force towards the target corner.
            attraction_force = diff.normalize() * 0.1 if dist > 1 else pygame.Vector2(0, 0)
            # Apply a damping force to help agents settle down.
            damping_force = self.velocity * -0.15
            
            self.velocity += attraction_force + damping_force
            if self.velocity.length() > AGENT_SPEED: self.velocity.scale_to_length(AGENT_SPEED)
            ...
            # NEW: Check for significant deviation from the path due to being pushed.
            # If the agent is too far from its intended path segment, it should replan
            # instead of wasting time trying to get back to a now-distant waypoint.
            if self.path_valid and self.current_wp_index > 0 and self.current_wp_index < len(self.path):
                p1 = self.path[self.current_wp_index - 1]
                p2 = self.path[self.current_wp_index]
                
                # Threshold for replanning, e.g., 2.5 times the agent's diameter.
                deviation_threshold = AGENT_DIAMETER * 2.5
                
                dist_to_path = _point_to_segment_dist(self.pos.x, self.pos.y, p1[0], p1[1], p2[0], p2[1])
                
                if dist_to_path > deviation_threshold:
                    self.recalc_path()
                    # After replanning, process the new path in the next frame.
                    return
            ...
            # Check for entering parking zone AFTER moving
